...
async def _replace_stop_job(job_id: str):
    ...
    if stop_index < 0 or stop_index >= len(stops):
        await debug_logger.log(LogLevel.ERROR, f"Ungültiger Stop-Index {stop_index}", job_id=job_id, agent="ReplaceStop")
        await debug_logger.push_event(job_id, "job_error", None, {"error": "Ungültiger Stop-Index"})
        return
    ...
    # --- Parallel research ---
    await debug_logger.push_event(job_id, "replace_stop_progress", None, {
        "stop_id": stop_id, "phase": "research", "message": "Aktivitäten, Restaurants & Bilder werden recherchiert…",
    })
    ...
    # --- Travel Guide ---
    await debug_logger.push_event(job_id, "replace_stop_progress", None, {
        "stop_id": stop_id, "phase": "guide", "message": "Reiseführer wird erstellt…",
    })
    ...
    # --- Accommodation ---
    await debug_logger.push_event(job_id, "replace_stop_progress", None, {
        "stop_id": stop_id, "phase": "accommodation", "message": "Unterkünfte werden gesucht…",
    })
    ...
    await debug_logger.push_event(job_id, "replace_stop_progress", None, {
        "stop_id": stop_id, "phase": "day_planner", "message": "Tagespläne werden neu berechnet…",
    })
